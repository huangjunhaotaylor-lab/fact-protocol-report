 ...
 *
 * G3：字典来源改为 loadDictionary()（优先持久化字典 data/domain-dictionary.json，
 * 缺失回退内置）；仍可显式传入 dictionary 参数保持纯函数可测。
 */
 ...
 * @param dictionary 板块字典（默认取当前生效字典：持久化优先、内置兜底）
 */
export function classifyDomains(
  text: string,
  dictionary: Record<string, string[]> = loadDictionary(),
): DomainClassification {
  const normalized = (text ?? '').toLowerCase();

  // 逐板块计分：去重命中，每词 1 分
  const scores: Record<string, number> = {};
  for (const [domain, keywords] of Object.entries(dictionary)) {
    let score = 0;
